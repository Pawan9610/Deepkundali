...
(function () {
  ...
  spring.addListener({
    onSpringUpdate: function onSpringUpdate(spr) {
      var val = spr.getCurrentValue();
      rot = rebound.MathUtil.mapValueInRange(val, 0, 1, 0, 40);
      var transform = 'rotateX(' + rot * force.y / 3 + 'deg) ' + 'rotateY(' + rot * force.x + 'deg)';
  ...
  function updateRotation(relX, relY) {
    ...
    spring.setEndValue(Math.max(Math.abs(force.x), Math.abs(force.y))); //spring.setEndValue(Math.sqrt(Math.pow(Math.abs(force.x), 2) + Math.abs(Math.abs(force.y), 2)));
  }
